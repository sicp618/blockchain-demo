...
export class NftMarket extends ethereum.SmartContract {
  ...
  getProcess(seller: Address): BigInt {
    let result = super.call("getProcess", "getProcess(address):(uint256)", [
  ...
  try_getProcess(seller: Address): ethereum.CallResult<BigInt> {
    let result = super.tryCall("getProcess", "getProcess(address):(uint256)", [
      ethereum.Value.fromAddress(seller)
    ]);
    if (result.reverted) {
      return new ethereum.CallResult();
